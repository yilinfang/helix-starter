...
# URLs for tools
HELIX_URL="https://github.com/helix-editor/helix/releases/download/25.01.1/helix-25.01.1-aarch64-macos.tar.xz"
...
main() {
	...
	if [ $UPDATE_SHELL_CONFIGURATION -eq 1 ]; then
		...
		if ! grep -q "source $PREFIX/init.zsh" "$ZSH_CONFIG_FILE"; then
			...
			echo "# helix-starter configuration" >>"$ZSH_CONFIG_FILE"
